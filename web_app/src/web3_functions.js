...
async function startNewElection(contractInstance, account, _electionName){
    try {
        let res2 = await contractInstance.methods.startNewElection(_electionName).send({from: account});
        console.log("Res:",res2);
        return {error: false, message: res2.events.success.returnValues.msg}
        } catch (error) {
            console.log("Error:",error);
...
async function getTotalNumVotes(contractInstance, account, _getTotalVotes){
    try {
